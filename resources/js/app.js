...
// Mobile Featured Rooms Carousel (CSS Scroll Snap + JS Indicators)
(() => {
    document.querySelectorAll('[data-featured-carousel]').forEach((carousel) => {
        ...
        if (!container || !indicatorsContainer || items.length === 0) return;
        ...
        const dots = items.map((item, index) => {
            ...
            btn.type = 'button';
            btn.className = 'w-2 h-2 rounded-full transition-all duration-300 bg-stone-300 focus:outline-none focus:ring-2 focus:ring-[#B6424F]';
                    ...
                    if (index !== -1) {
                        dots.forEach((dot, i) => {
                            if (i === index) {
                                ...
                                dot.classList.add('bg-[#B6424F]', 'w-6');
                                dot.setAttribute('aria-current', 'true');
                            } else {
                                dot.classList.add('bg-stone-300', 'w-2');
                                dot.classList.remove('bg-[#B6424F]', 'w-6');
        ...
        if (dots.length > 0) {
            dots[0].classList.remove('bg-stone-300', 'w-2');
            dots[0].classList.add('bg-[#B6424F]', 'w-6');
        }
        ...
        if (prevBtn) {
            prevBtn.addEventListener('click', () => {
                ...
                scrollToIndex(activeIndex > 0 ? activeIndex - 1 : 0);
            });
        }
        ...
        if (nextBtn) {
            nextBtn.addEventListener('click', () => {
                const activeIndex = dots.findIndex(dot => dot.getAttribute('aria-current') === 'true');
                const nextIndex = activeIndex !== -1 ? activeIndex + 1 : 1;
                scrollToIndex(nextIndex < items.length ? nextIndex : items.length - 1);
            });
        }
    });
})();
